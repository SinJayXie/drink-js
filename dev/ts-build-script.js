...
function start_build() {
    ...
    tscProcess.on('exit', (code) => {
        if(code === 0) {
            ...
            exists(path.join(__dirname, '..' , 'src/templates'),path.join(__dirname, '..' , 'build/templates'), copy);
            exists(path.join(__dirname, '..' , 'src/static'),path.join(__dirname, '..' , 'build/static'), copy);
        }
    });
}
